...
def logResult(res):
    file = open("log.txt", "a")
    empty = str(res["asr_result"]["result"]) == ""
    if not empty:
        file.write(str(res["asr_result"]["result"]) + "->" + str(res["entities"][0]["values"]) + "\n")
    file.close()
...
class Dialog(object):
        ...
        try:
            while True:
                ...
                if result_task in done:
                    ...
                    # THIS WAS ADDED BY TOMÁŠ LEBEDA - I want the result back event if there is no recognized SLU entity
                    logResult(result)
                    emptyResult = str(result["asr_result"]["result"]) == ""
                    unrecognizedResult = str(result["entities"][0]["values"]) == "[]"
                    if not emptyResult and unrecognizedResult:
                        print("BREAKING: ", emptyResult, unrecognizedResult)
                        pprint(result)
                        return result
    ...
    async def recognize_and_wait_for_slu_result(self, timeout=None):
        logging.debug("Entered recognize_and_wait_for_slu_result")
        await self.sc.asr_recognize()
        logging.debug("Passed self.sc.asr_recognize")
        try:
            logging.debug("entered try for wait_for_slu_result")
            return await self.wait_for_slu_result(timeout)
        finally:
            logging.debug("entered finally block on asr_pause")
            await self.sc.asr_pause()
